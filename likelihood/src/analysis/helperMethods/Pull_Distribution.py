...
def createHistogram(data_points, True_value, hist_range, bin_width, method_id, output_folder = None):
    fig, ax = plt.subplots(1, 1, figsize=(5, 4), dpi=1000)
    ...
    ax.plot(x_points, gaussian(x_points, mean, sigma), label="Normal Best-Fit")

    ax.set_title(Title)
    ax.set_xlabel(r"$(\hat\tau-\tau_0) / \sigma_{\hat\tau}$")
