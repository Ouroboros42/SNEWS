...
void test_row_lead_term(FactorialCache cache, DetectorComparison comp, size_t n, size_t m) {
...
void test_lead_term(FactorialCache cache, DetectorComparison comp, size_t n, size_t m) {
    cache.build_upto(n + m);
    auto big_bin = log_sum_terms(cache, comp, n, m);
...
TEST_CASE("Bin likelihood row lead term") {
    ...
    DetectorComparison comp(b1, b2, ratio);
